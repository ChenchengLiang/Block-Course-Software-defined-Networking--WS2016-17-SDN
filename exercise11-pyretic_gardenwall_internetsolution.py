        ...
        for f in self.flow_state_map:
            ...
            if srcmac_field is not None:
                ...
                if infected_state == 'True':
                    this_policy = if_(match(srcmac=EthAddr('00:00:00:00:00:01')), drop,passthrough)>>if_(match(srcmac=EthAddr('00:00:00:00:00:03')), passthrough, )	
                    print "Infected"                
                # Forward to gardenwall if both True.
                if infected_state == 'True' and exempt_state == 'True':
                    print "Install garden wall"
                    garden_mac = EthAddr('00:00:00:00:00:03')
                    garden_ip = IPAddr('10.0.0.3')

                    change_ip_policy = (modify(dstip=garden_ip))
                    change_mac_policy = (modify(dstmac=garden_mac))
                    this_policy = if_(match(dstmac=EthAddr('00:00:00:00:00:02')),change_mac_policy>>passthrough,)

                    #this_policy = match(change_mac_policy>>dstmac=EthAddr('00:00:00:00:00:02'))>>passthrough
                ...
                # Else if infected is True, drop.
                    ...
                    #change_ip_policy = (modify(dstip=garden_ip))
                    #change_mac_policy = (modify(dstmac=garden_mac))
                    #this_policy = match(dstmac=EthAddr('00:00:00:00:00:02'))>> change_mac_policy>>passthrough	
                    #this_policy = match(srcmac=EthAddr('00:00:00:00:00:02'))>>modify(dstmac=EthAddr('00:00:00:00:00:03'))>>passthrough
                    #this_policy = match(srcmac=EthAddr('00:00:00:00:00:02'))>>passthrough	
                    #this_policy = if_(match(srcip=IPAddr('10.0.0.2'), dstip=IPAddr('10.0.0.3')), passthrough, modify(srcip=IPAddr('10.0.0.2'), dstip=IPAddr('10.0.0.3')))
                    #this_policy = if_(match(srcip=IPAddr('10.0.0.3')), passthrough, modify(dstip=IPAddr('10.0.0.3')))
                    #this_policy = if_(match(srcmac=EthAddr('00:00:00:00:00:01')), drop,passthrough)>>if_(match(srcmac=EthAddr('00:00:00:00:00:03')), passthrough, match(dstip=IPAddr('10.0.0.3')))	
                    print "Infected"
                    #print "Install drop rule"
                    #this_policy = drop
